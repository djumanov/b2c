...
Anything that has to know what GTS did is deliberately **polling**. GTS.md §12
documents only the B2C→GTS direction, so we cannot rely on GTS calling us. If
it turns out that it can, a webhook becomes an optimisation rather than a
rewrite (ARCHITECTURE.md §12).
"""
...
celery_app.conf.update(
    ...
    timezone="UTC",
    enable_utc=True,
    # A task that dies with its worker must come back, not vanish
    # (ARCHITECTURE.md §8).
    task_acks_late=True,
