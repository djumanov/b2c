...
@router.post(
    ...
    description=(
        ...
        "own currency — 294 EUR beside an order priced in UZS. That figure is "
        "passed through for the breakdown; it is not what this order costs, "
        "and it is never charged. Everything this API prices, charges and "
        "books is in **UZS**; a price GTS says has *moved* in any other "
        "currency is refused with a `502` rather than converted.\n\n"
...
@router.post(
    ...
    description=(
        ...
        "* `awaiting_otp` — **the code was refused** (mistyped, expired), "
        "and that is all: the provider checks the code before it moves any "
        "money, so nothing was charged and nothing is over. `payment.error` "
        "is the reason to show; `payment_id`, the card and `phone_hint` are "
        "unchanged. Call this endpoint again with another code, or "
        "`payment/resend/` for a fresh SMS. Step 1 is **not** repeated — a "
        "new step 1 would register the card again.\n"
        "* `failed` — the charge itself was refused: insufficient funds, "
        "card blocked, provider's verdict in `payment.error`. Nothing was "
        "charged; start again from step 1.\n"
        ...
        "pending is a read. Retries carry their own `Idempotency-Key`, or "
        "none at all — the derived key follows the body, so a different code "
        "is a different request."
    ),
    response_description=(
        "The order with `payment.status` `paid`, `awaiting_otp` (the code was "
        "refused — try another), `failed` or `processing`."
    ),
    responses=error_responses(
        ErrorCode.CONFLICT,
        ErrorCode.OFFER_EXPIRED,
...
@router.post(
    ...
    description=(
        ...
        "step 1 (or from a previous resend). An attempt whose code step 2 "
        "just refused is open like any other: this is the way out of a code "
        "that never arrived or no longer works.\n\n"
