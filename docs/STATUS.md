...
| 14d | `GET /public/orders/` **qisqartirilgan**: 11 maydon, `data` va `passengers` yo'q, `passenger_count` bor | `bd89f96` "ro'yxat butun qatorni qaytaradi" degan edi, asosi: "`data` baribir payload'ni egallaydi". Endi `data` ning **o'zi** ro'yxatdan chiqdi, ya'ni asos yo'q. Ikki foyda: sahifada 20 ta GTS blobi yuborilmaydi, va pasport raqamlari har ro'yxat so'rovida uchmaydi (§13). So'rov ham `defer(provider_response)` bilan blobni Postgres'dan tortmaydi |
