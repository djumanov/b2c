...
- `passengers.document_type` va `citizenship` endi **JSONB katalog obyekti** —
  ...
  `"code"`/`"type"` kalitini tekshiradi (§4.74). **2-faza kuzatuvi:** bron oqimi
...
| 39 | ~~`document_type` — **cheklanmagan satr**~~ — **№74 bilan qisman bekor** (endi JSONB obyekt); enum/CHECK yo'qligi kuchda | Katalog GTS tomonda. §26 `document-types/` endi uni klientga beradi, lekin ustunni cheklamaydi: lokal enum GTS ro'yxati o'zgarganda unga zid bo'lib chiqadi, CHECK esa bu xatoni tuzatishni migratsiyaga aylantiradi |
...
| 68 | ~~`passengers.citizenship` — **cheklanmagan satr**~~ — **№74 bilan qisman bekor** (endi JSONB obyekt); enum/ISO tanlamaslik kuchda | `document_type` bilan bir sabab: davlatlar katalogi GTS tomonda va §26 `countries/` uni faqat **ko'rsatadi**, cheklamaydi — lokal ro'yxat GTS ro'yxati o'zgarganda unga zid bo'lib chiqardi. Ustiga kod tanlash standartni ham tanlash demak — "UZ" mi, "UZB" mi, kontrakt hali aytmagan. Satrni keyin kengaytirish arzon, upstream bilan kelishmaydigan CHECK'ni yechish esa yo'q |
