...
**Oxirgi yangilanish:** 2026-08-19 · `main` (karta oqimi + buyurtma marshruti)
...
| Migratsiyalar | 37 ta, bitta head (`7c4e2b915da8`) |
| Testlar | 1212 ta — unit 26 fayl · contract 10 · integration 38 |
...
| `providers/products/flight.py` | `flight` adapteri: **butun oqim** — `search`, `offers`, `upsell`, `verify`, `booking`, `cancel`; sof passthrough — tana yengil tekshiruvdan so'ng GTS'ga aynan, javob `data` si kelganicha (API.md §20, 2026-08-12 va 2026-08-14 qarorlari). `booking`/`cancel` timeout'i 15 s (§12), javobga hech narsa qo'shilmaydi; `cancel` da shakl tekshiruvi umuman yo'q. `offers`/`upsell` — yagona istisno: upstream nosozlik `502` emas, **bo'sh natija** bo'lib qaytadi (API.md §20, 2026-08-18 qarori) | — |
...
| `orders` (public) | `GET /public/orders/` va `/{id}/` — mijozning o'z buyurtmalari, `?product=`/`?status=` filtri bilan. Qator bron **niyatidan** tug'iladi (S3), GTS javobi esa `JSONB` bo'lib kelganicha yoniga saqlanadi; adapter undan identifikatorlarni, narxni, yo'lovchilarni (`travelers`) va marshrutni (`route`, `route_summary`, `travel_start_at`/`travel_end_at`) ustunga ajratadi. Status — **kanonik**, GTS kodi `provider_status` da yonida turadi. Ro'yxat karta: marshrut, sanalar, qisqartirilgan yo'lovchilar va narx; `data` va pasport ma'lumoti faqat `{id}/` da (qaror 14e) | 2 |
...
| 14a | `Idempotency-Key` — **majburiy emas**: klient yubormasa server uni so'rovdan deterministik hosil qiladi (`auto:` + sha256(sub'ekt·metod·yo'l·normallashtirilgan tana)) | Kalitni majburiy qilish himoyani klientning holat boshqaruviga bog'lardi — kalitni ekran state'ida saqlash, retry'da o'shani yuborish, forma o'zgarganda yangilash — va har bir qoidaning buzilish narxi ikkinchi o'rin yoki ikkinchi to'lov. **Tasodifiy** kalit yaratish rad etildi: u har so'rovga boshqa qiymat berib himoyani butunlay yo'q qilardi. Tana solishtirishdan oldin JSON sifatida normallashtiriladi, aks holda serializatsiya farqi ikkinchi bron bo'lardi |
| 14b | Sim ustida `gts_*` (`gts_order_number`, `gts_order_uid`, `gts_status`, `gts_pnr`) va `passengers`; ustunlarda `provider_*` va `travelers` | Buyurtma tizimi qurilganda kontrakt maydonlari qayta nomlangan edi va API klientlar buzilgan kodlari haqida shikoyat qildi. Ichkarida `provider_*` to'g'ri (tizim mahsulot-agnostik), lekin **tashqarida u hech narsa bermadi** — GTS bitta provayder, klientlar esa eski nomlarni bilishadi. `travelers` esa umuman asossiz edi: so'rov ham, §19 ham, GTS ham `passengers` deydi. O'girish `OrderOut.from_order` da, migratsiya kerak emas |
| 14c | Bron javobi **additiv**: GTS javobi o'z joyida, `order` va `payment` uning yoniga qo'shiladi | Avval GTS javobi `data` ning ichiga tushirilgan edi, ya'ni `data.data.order_number` → `data.data.data.order_number`. Bu klient kodini sababsiz buzardi: qo'shimcha maydon qo'shish buzuvchi o'zgarish emas (§1), butun javobni bir qavat pastga tushirish esa buzuvchi. To'qnashuv (GTS `order` kaliti yuborsa) log'ga yoziladi |
| 14d | `GET /public/orders/` **qisqartirilgan**: 11 maydon, `data` va `passengers` yo'q, `passenger_count` bor. **14e qisman almashtirdi** | `bd89f96` "ro'yxat butun qatorni qaytaradi" degan edi, asosi: "`data` baribir payload'ni egallaydi". Endi `data` ning **o'zi** ro'yxatdan chiqdi, ya'ni asos yo'q. Ikki foyda: sahifada 20 ta GTS blobi yuborilmaydi, va pasport raqamlari har ro'yxat so'rovida uchmaydi (§13). So'rov ham `defer(provider_response)` bilan blobni Postgres'dan tortmaydi |
| 14e | Karta yana kengaydi: `route` (yo'nalish darajasidagi obyekt), `passengers` (**besh maydon**: ism, familiya, ota ismi, turi, chipta raqami) va `travel_end_at` qo'shildi. `data` hamon yo'q, hujjat/tug'ilgan sana/kontakt ham yo'q | 14d ikkita sababni bitta qarorga qo'shib yuborgan edi. Blob haqidagi sabab kuchida: 20 ta GTS javobi hech kim o'qimaydigan yuz kilobayt. Yo'lovchi haqidagi sabab esa **maydon darajasida**, obyekt darajasida emas — uchadigan narsa pasport raqami, ism emas. Ro'yxat ekrani kimning nomiga bron qilinganini va qayerga uchishini ko'rsatishi kerak, va buning uchun har bir kartadan `{id}/` ga borish 20 ta qo'shimcha so'rov degani. `route` ustunga chiqdi, chunki aks holda javob blobni ochishni talab qilardi — ya'ni 14d ning kuchida qolgan yarmini buzardi |
