...
app/
  ...
  modules/                 # funksional bo'laklar; har birida: router_public.py,
    ...
    customers/             # akkaunt, profil, yo'lovchilar, qurilmalar
    ...
    payments/              # to'lovlar, tranzaksiyalar, qaytarishlar, kartalar, webhook
...
| ~~`booking`~~ | **Yo'q qilindi (2026-08-18).** Saga `orders` moduliga ko'chdi: holat mashinasi va uni haydaydigan saga birga o'zgaradi, ajratilsa modullararo sikl paydo bo'ladi ([order-system/03-design.md](order-system/03-design.md) `O1`) |
| `orders` | Buyurtma **aggregate**'i: holat mashinasi, saga, yo'lovchilar, status tarixi, `sync`, `available_actions` | D3 bo'yicha — **eng yuqori xavfli modul**. Kanonik status bizniki va `CHECK` bilan qo'riqlanadi; GTS kodi yonida `provider_status` da qoladi ([order-system/03-design.md](order-system/03-design.md) §3.3) |
