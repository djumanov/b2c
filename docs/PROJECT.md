...
| **D7** | To'lov provayderlari — **Payme + Click**, ikkalasi ham **karta API'si orqali**: mijoz karta ma'lumotini bizning formamizga kiritadi, provayder SMS kod yuboradi, server yechadi. Redirect **yo'q** — hech kim provayderning sahifasiga ketmaydi. Qaysi provayder yechishini **egasi paneldan tanlaydi**: bir vaqtda bittasi yoqilgan. **Saqlangan kartalar provayderga bog'lanmaydi**: karta — lokal autofill yozuvi, raqami bazada **faqat AES-GCM shifrlangan holda** turadi va to'lov paytida server tomonidan ochilib provayderga uzatiladi ([API.md](API.md) §19, §22). ⚠ Shifrlangan bo'lsa ham raqamni saqlash PCI qamrovini og'irlashtiradi — §13. *(Uch marta qayta ko'rib chiqildi: dastlab redirect-only; keyin saqlangan kartalar talabi bilan karta-token oqimiga kengaytirildi; 2026-08-11 da saqlash soddalashtirildi — provayder tokeni o'rniga shifrlangan lokal yozuv; **2026-08-19 da to'lovning o'zi kartaga o'tdi va redirect olib tashlandi** — `order-system/03-design.md` `O14`, `O15`. ⚠ Redirect bilan birga **hamyon bilan to'lash** ham ketdi: Payme hamyonidagi pulni endi sarflab bo'lmaydi)* |
...
| To'lov provayderi timeout'i | Kartani ro'yxatdan o'tkazish va tasdiqlash **15 s**, yechish **20 s**. ⚠ Yechish bitta HTTP so'rovning ichida bo'ladi ([API.md](API.md) §22) va Payme uchun bu ikkita provayder aylanmasi — **ingress/proksi timeout'i ataylab 20 s dan yuqori qo'yilsin**, aks holda mijoz uzilgan ulanish oladi, holbuki pul yechilgan bo'lishi mumkin | D7 |
...
| Mavzu | Qoida |
|---|---|
| **Karta raqami** | **Hech qachon ochiq matnda saqlanmaydi, log'ga tushmaydi va klientga qaytmaydi.** Bazada faqat **AES-GCM shifrlangan** nusxa turadi (`key_version` bilan — integratsiya kalitlari bilan bir xil naql); u faqat to'lov paytida server ichida ochiladi va provayderga uzatiladi. Yangi karta bilan to'lashda raqam so'rov tanasidan adapterga o'tadi va **hech qayerga yozilmaydi** — jumladan idempotentlik kalitiga ham, shuning uchun karta yo'lida `Idempotency-Key` yo'q ([API.md](API.md) §22). **CVV umuman so'ralmaydi**: na Payme, na Click uni talab qiladi, so'ralmagan maydon esa himoyalanishi shart bo'lmagan maydon |
| **Karta yozuvi** | Shifrlangan raqam yonida faqat ko'rsatish uchun kerak bo'lgan maskalangan raqam, oxirgi to'rt raqam, BIN, brend va amal muddati turadi — bularning hammasi raqamning o'zidan lokal chiqariladi, provayder qatnashmaydi |
| **To'lov urinishi** | Urinish qatorida kartaning **maskalangan** nusxasi, oxirgi to'rt raqami va brendi qoladi — kvitansiya karta unutilgandan keyin ham o'qiladi. Ustiga provayderning **bir martalik tokeni**, AES-GCM shifrlangan, va faqat urinish ochiq turgan vaqtda: yakunlangan qatorda token `NULL` bo'lishini CHECK majbur qiladi. SMS kod hech qachon saqlanmaydi va log'ga tushmaydi |
| **PCI DSS** | Karta raqami — shifrlangan bo'lsa ham — **saqlanadi**, shuning uchun o'rnatma **SAQ D** qamrovida, ustiga **saqlangan karta ma'lumoti bilan**: PCI 3-talab (saqlash nazorati, hujjatlashtirilgan kalit boshqaruvi, saqlash muddati va o'chirish siyosati) to'liq qo'llanadi, yillik o'z-o'zini baholash, choraklik ASV skani, pen-test va hodisaga javob rejasi bilan birga. Bu avvalgi token-oqimidan **og'irroq** qamrov. 2026-08-19 dan boshlab raqam ustiga **jonli kanaldan ham o'tadi** (D7), ya'ni TLS, jurnal va tarmoq nazoratlari ham qamrovga kiradi — toifa o'zgarmadi, yuzasi kengaydi. Majburiyat **o'rnatma egasining zimmasida** (D10) — §16.7 |
...
**Akkaunt o'chirilganda** (`DELETE /public/profile/`) jonli qatordagi shaxsiy ma'lumot
tozalanadi, lekin buyurtma va to'lov yozuvlari **moliyaviy hujjat sifatida anonimlashtirilib
saqlanadi** — ular hisobot va qaytarish uchun kerak. Saqlangan kartalarning **shifrlangan
...
jonli tizimdan chiqarish ma'nosida qoladi, biznes esa kimlar va nima sababdan ketganini
ko'ra oladi. Arxiv faqat bazada, hech bir API uni qaytarmaydi.

> Saqlash muddatlari (audit log, anonimlashtirilgan buyurtmalar, `deleted_customers`
> arxivi) hali belgilanmagan — §16.
...
| **2. GTS ulanishi va birinchi vertikal** | GTS sessiya klienti va anti-corruption qatlami, `ProductAdapter` porti, **aviachipta** adapteri, holatsiz qidiruv oqimi (D2), verify/bron, Payme + Click, **saqlangan karta bilan to'lov (D7)**, saga, buyurtmalar, **to'lovga promokod qo'llash** | Aviachipta bo'yicha qidiruv → bron → to'lov → chipta uchidan-uchiga ishlaydi · chipta xatosida avtomatik qaytarish ishlaydi · takroriy webhook ikki marta yechmaydi · saqlangan karta bilan to'lov ishlaydi va **karta raqami ochiq matnda hech bir jadvalda yo'q** |
...
| 7 | **PCI DSS majburiyati** — SAQ D ni kim to'ldiradi, ASV skanini kim buyurtma qiladi, natijani kim saqlaydi? ⚠ Bu **tijoriy javob**, texnik emas. *2026-08-11 texnik qaror qabul qilindi: karta raqami AES-GCM shifrlangan holda saqlanadi (D7) — ya'ni qamrov endi "saqlangan karta ma'lumoti bilan SAQ D", §13.* Tijoriy yarmi ochiq qoladi va endi saqlash nazorati (PCI 3-talab) ni ham o'z ichiga oladi. *2026-08-19: to'lovning o'zi ham kartaga o'tdi (D7), ya'ni raqam endi saqlanadi **va** jonli kanaldan o'tadi — toifa o'zgarmadi, nazorat yuzasi kengaydi.* Rad etilsa muqobil bor, lekin u endi qimmatroq: kartani provayderning o'z formasida ro'yxatdan o'tkazish — token keladi, raqam saqlanmaydi, evaziga mijoz saytdan chiqadi va O14 bekor qilinadi | §13, D7, to'lovlar moduli |
