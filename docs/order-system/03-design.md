...
|---|---|---|
| **O1** | **Order — yagona aggregate root.** Holat mashinasi ham, saga ham `orders` modulida. `app/modules/booking/` placeholderi **o'chiriladi** | Muqobil: `ARCHITECTURE.md` §5 dagidek saga `booking/` da. Rad etildi: holat mashinasi va uni haydaydigan saga birga o'zgaradi; ajratilsa outbox `orders` da yoziladi-yu `booking` dagi tasklarni uyg'otadi — modullararo sikl. Bu `ARCHITECTURE.md` §5 dan **qayd etilgan chetlanish** |
...
| **O9** | **Event sourcing yo'q** — append-only status tarixi + xom javob snapshot'lari | `01-research.md` §1.8 da asoslangan (X10) |
| **O10** | Yo'lovchilar **normallashtiriladi**, blobda qolmaydi | A8 ni yopadi: `PROJECT.md` §13 anonimlashtirish va'dasi faqat shunda bajariladi |
...
| **O12** | Bir buyurtmaga **bitta** to'lov; to'lov ostida **ko'p** tranzaksiya | Stripe naqshi (X2). Mijoz Payme'da rad etilib Click bilan to'lasa — ikkinchi urinish, ikkinchi to'lov emas |
...
```mermaid
erDiagram
    ORDERS ||--o{ ORDER_TRAVELERS : "yo'lovchi / mehmon"
    ORDERS ||--o{ ORDER_EVENTS : "status tarixi"
    ORDERS ||--o{ ORDER_SNAPSHOTS : "provider xom javoblari"
    ORDERS ||--o| PAYMENTS : "v1 — bittaga bitta"
    PAYMENTS ||--o{ PAYMENT_TRANSACTIONS : "urinishlar"
    ORDERS ||--o{ REFUNDS : "auto / so'rov bo'yicha"
    REFUNDS }o--|| PAYMENTS : "qaysi puldan"
    OUTBOX }o--|| ORDERS : "aggregate_id"
...
| Entity | Kim yaratadi | Nima uchun alohida |
|---|---|---|
| `Order` | Biz | Yagona aggregate root: holat mashinasi, egalik, pul yig'indisi (X1) |
| `OrderTraveler` | Biz, GTS javobidan | Chipta raqami **yo'lovchiga** tegishli; anonimlashtirish shu yerda bajariladi (O10) |
| `OrderEvent` | Biz | Har bir o'tishning append-only izi (O9) |
| `OrderSnapshot` | Biz | Provayder javobining xom dalili — nizolarda va tiklashda (O9) |
| `Payment` | Biz | Umri buyurtmanikidan boshqa; kelajakda "keyin to'lash" (X2) |
| `PaymentTransaction` | Biz, provayder bajaradi | Bitta to'lovga ko'p urinish (O12) |
| `Refund` | Biz | Auto va so'rov bo'yicha — bitta jadval, ikki kirish yo'li (O11) |
| `OutboxMessage` | Biz | Yon ta'sirni holat o'zgarishi bilan bitta tranzaksiyada yozish (X5) |

**Bron (booking) alohida entity emas.** U — buyurtmaning provayderdagi aksi:
identifikatorlari `orders` ustunlarida (`provider_order_number`, `provider_pnr`,
`ticket_time_limit_at`), xom javobi `order_snapshots` da. Alohida jadval
qilinsa, u `orders` bilan bir-biriga bir yozuvda bog'lanib, faqat `JOIN`
qo'shardi.
...
| # | Dan → Ga | Trigger | Guard | Bitta tranzaksiyadagi yon ta'sir | Guard buzilsa |
|---|---|---|---|---|---|
| **T1** | ∅ → `created` | `POST /{product}/booking/` | `Idempotency-Key` bor; `offer_id` bor; mijoz autentifikatsiyadan o'tgan | — | `422` |
| **T2** | `created` → `booked` | GTS `booking` muvaffaqiyatli | `provider_order_number` **va** summa/valyuta o'qildi | `payments` qatori yaratiladi · `order_travelers` to'ldiriladi · snapshot · `notify.order_booked` | O'qilmasa → `needs_attention`, snapshot bilan |
| **T3** | `created` → `failed` | GTS `status: "error"` | — | snapshot | — |
...
| **T10** | `ticketing` → `ticketed` | GTS `ticketing` muvaffaqiyatli | **Har bir** yo'lovchida chipta raqami bor | `notify.order_ticketed` · snapshot | Qisman chiqsa → `needs_attention` |
...
6. yon ta'sirlar `outbox` ga yoziladi;
7. `COMMIT`.

Task yuborish **hech qachon** shu tranzaksiya ichida bo'lmaydi — buni
`outbox.dispatch` commitdan keyin qiladi (X5).
...
Konvensiyalar `ARCHITECTURE.md` §10 dan: UUID PK (ilova tomonda),
`created_at`/`updated_at`/`deleted_at`, pul — `NUMERIC(18,2)` + alohida
`CHAR(3)`, nomlash konvensiyasi `db/base.py` dan.

> **CHECK cheklovlari qo'lda yoziladi.** Alembic autogenerate ularni
> o'tkazib yuboradi — har bir migratsiyada `CheckConstraint` qo'lda
> qo'shiladi va `downgrade()` da o'chiriladi.

### `orders`

| Ustun | Tur | Izoh |
|---|---|---|
| `id` | UUID PK | Ilova tomonda `uuid4` — outbox qatorini **bitta tranzaksiyada** yozish uchun id insertdan oldin kerak (`db/mixins.py:30-32`) |
...
| `provider_pnr` | `String(32)` | `gds_pnr`. Admin qidiruvi shu bo'yicha ishlaydi (`API.md` §31) |
...
| `amount_paid` | `NUMERIC(18,2)` | Sukut `0` |
| `amount_refunded` | `NUMERIC(18,2)` | Sukut `0` |
| `ticket_time_limit_at` | `timestamptz` | Bron muddati. `ticket_deadline` shundan hisoblanadi |
| `void_deadline_at` | `timestamptz` | v2 |
| `travel_start_at`, `travel_end_at` | `timestamptz` | Mahsulotdan qat'i nazar: reys/kirish sanasi. Hisobot va eslatmalar uchun |
...
| `failure_code`, `failure_message` | `String(64)`, `Text` | GTS rad etgan sabab |
| `attention_reason` | `String(64)` | Nega qo'lda hal qilish kerak |
| `booked_at`, `paid_at`, `ticketed_at`, `cancelled_at`, `refunded_at` | `timestamptz` | Har bir muhim o'tishning vaqti — hisobotlar uchun `order_events` ni skanerlamaslik |
...
ix_orders_provider_pnr             (provider_pnr)                     -- admin qidiruvi
...
ix_orders_open                     (status, updated_at)
                                   WHERE status IN ('created','ticketing','refunding','needs_attention')
```

Oxirgi ikkitasi — **qisman indeks**: supurgich va stuck-monitor faqat kichik
qismni skanerlaydi, jadval o'sganda ham arzon qoladi.

**`deleted_at` yozilmaydi.** Buyurtma — moliyaviy hujjat (`PROJECT.md` §13),
u o'chirilmaydi, **anonimlashtiriladi**. Ustun `Entity` dan meros qoladi va
har o'qishda hisobga olinadi, lekin uni hech bir kod yozmaydi (A11).

### `order_travelers`

`order_id` FK (`ON DELETE CASCADE`) — **bir modul ichida**, shuning uchun FK
ruxsat etilgan. Ustunlar: `position` · `type` (`ADT`/`CHD`/`INF`/`INS`) ·
`first_name` · `last_name` · `middle_name` · `birth_date` · `gender` ·
`citizenship` · `document_type` · `document_number` · `document_issue_date` ·
`document_expiry_date` · `email` · `phone` · `provider_traveler_id` ·
`ticket_number` · `ticket_status` · `anonymized_at`.

`UNIQUE (order_id, position)`. `ix_order_travelers_ticket_number`.

> **Bu jadval A8 ni yopadi.** PII endi ma'lum ustunlarda; akkaunt
> o'chirilganda ism, hujjat, email va telefon tozalanadi, `anonymized_at`
> qo'yiladi, chipta raqami va yo'lovchi turi **qoladi** — ular moliyaviy
> hujjatning bir qismi.

### `order_events`

Append-only. `Entity` emas — `AuditLog` kabi `Base + UUIDPrimaryKeyMixin`:
`updated_at` ham, `deleted_at` ham ma'nosiz.

`order_id` FK · `created_at` · `from_status` · `to_status` · `action` ·
`actor_type` (`system`/`customer`/`staff`) · `actor_id` · `actor_label` ·
`reason` · `meta` JSONB.
Indeks: `(order_id, created_at)`.

### `order_snapshots`

`order_id` FK · `created_at` · `kind` (`booking`/`retrieve`/`ticketing`/
`cancel`/`reprice`/`refund_quote`/`refund_commit`/`void`) · `direction`
(`request`/`response`) · `provider_status` · `payload` JSONB · `pii_purged_at`.

**Faqat brondan boshlab.** `search`/`offers`/`upsell`/`verify` javoblari bu
yerga **tushmaydi** — D2 buzilmaydi va `test_search_passthrough.py` yashil
qoladi.

### `payments`

`order_id` (UUID, **FK yo'q** — boshqa modul) · `customer_id` · `amount` ·
`currency` · `discount_amount` · `promo_code_id` · `status` · `method` ·
`expires_at` · `paid_at`.

CHECK: `status IN ('pending','processing','paid','failed','cancelled',
'refunded','partially_refunded')`.
`uq_payments_order_live` — `(order_id) UNIQUE WHERE deleted_at IS NULL` (O12).

`expires_at` = `min(ticket_deadline, created_at + payment_window)`;
`payment_window` — DB sozlamasi.

### `payment_transactions`

`payment_id` FK · `provider` · `provider_ref` · `flow` (`redirect`/`card`) ·
`status` · `amount` · `currency` · `redirect_url` · `error_code` ·
`error_message` · `provider_state` JSONB.

`uq_payment_transactions_provider_ref` — `(provider, provider_ref) UNIQUE
WHERE provider_ref IS NOT NULL`. **Takroriy webhook ikki marta yechmasligining
poydevori shu indeks**, mantiq emas (X6).

Qator **tarmoq chaqiruvidan oldin** commit qilinadi (`ARCHITECTURE.md` §8):
jarayon o'rtada o'lsa, `provider_ref` siz qolgan qator `payments.reconcile`
uchun dalil bo'ladi.

### `refunds`

`orders` modulida yashaydi: qaytarish — buyurtma umrining qadami, uning pul
oyog'ini `payments_service` bajaradi.

`order_id` FK · `payment_id` (UUID, FK yo'q) · `kind` (`auto`/`customer`/
`admin`) · `status` · `scope` JSONB (v2) · `quoted_total` · `penalty_amount` ·
`refund_amount` · `currency` · `quote_expires_at` · `provider_refund_ref` ·
`provider_order_refund_ref` · `requested_by` · `approved_by` · `reason` ·
`failure_code` · `failure_message` · `idempotency_key`.

CHECK: `status IN ('requested','approved','rejected','processing','succeeded',
'failed')`; `refund_amount >= 0`.
`uq_refunds_idempotency_key` UNIQUE partial.
`ix_refunds_pending` — `(status)` `WHERE status IN ('requested','approved','processing')`.

### `outbox`

`orders` modulida. Boshqa modullar unga **to'g'ridan-to'g'ri yozmaydi** —
`orders_service.transition()` orqali yozadilar (§4 qoidasi).

`aggregate_type` (`order`/`refund`) · `aggregate_id` · `kind` (task nomi) ·
`payload` JSONB · `status` (`pending`/`dispatched`/`failed`) · `attempts` ·
`available_at` · `dispatched_at` · `last_error`.
Indeks: `ix_outbox_ready (status, available_at)`.
...
Qolgani — `routes`/`segments`, xona turi, tur dasturi — `order_snapshots`
...
class OrderOperations(Protocol):
    ...
    async def cancel(self, client, ref) -> CancelResult: ...
    async def retrieve(self, client, ref) -> RetrieveResult: ...
    async def reprice(self, client, ref) -> RepriceResult: ...
    async def refund_quote(self, client, ref, scope) -> RefundQuote: ...
    async def refund_commit(self, client, ref, scope) -> RefundCommitResult: ...
...
class BookingResult:
    ...
    raw: dict[str, Any]  # snapshot uchun
```
...
2. **`ticket_time_limit` uch xil ko'rinishda kuzatilgan** — ISO datetime
   ...
   `created_at` ga qo'shiladi). Aniqlanmasa — DB sozlamasidan zaxira qiymat
   va `WARNING` log. **Bu taxmin; jonli GTS'da tekshirilishi shart** (§3.10).
...
sequenceDiagram
    ...
    A->>DB: T2 booked + payment + travelers + snapshot + outbox
    A-->>M: {order, payment, data}

    M->>A: POST payments/{id}/transactions/
    A->>DB: transaction (created) + COMMIT
    A->>P: to'lovni boshlash
    A-->>M: redirect_url
    M->>P: to'laydi
    P->>A: webhook
    A->>DB: transaction=paid, payment=paid, T5 → paid, outbox[orders.ticket]
    A-->>P: provayder kutgan javob

    OB->>W: orders.ticket(order_id)
    W->>G: reprice_check
    W->>G: /v1/content/ticketing/ (deposit)
    G-->>W: chipta raqamlari
    W->>DB: T10 → ticketed, travelers.ticket_number, outbox[notify]
```
...
sequenceDiagram
    ...
    alt retryable va deadline ichida
        W->>DB: T9 (ticketing'da qoladi, attempts++)
        Note over W: backoff bilan qayta
    else terminal / deadline / narx oshdi
        W->>DB: T11 → refunding + refunds(kind=auto, approved)
...
> **Diqqat:** ticketing ishdan chiqqanda GTS tomonidagi kompensatsiya —
> **`cancel`**, `refund-commit` emas. Chipta chiqmagan, demak qaytariladigan
> hujjat yo'q; bo'shatiladigan narsa — bron. `refund-commit` faqat
> **chiptalangan** buyurtmada ishlatiladi (T13).

### Tasklar

| Task | Ishga tushishi | Navbat | Idempotentligi |
|---|---|---|---|
| `outbox.dispatch` | beat, 10 s | `money` | `status='pending' AND available_at<=now()` ni `FOR UPDATE SKIP LOCKED` bilan oladi |
| `orders.ticket` | outbox | `money` | Status `paid`/`ticketing` bo'lmasa jimgina chiqadi |
| `orders.cancel_provider` | outbox | `money` | GTS'da allaqachon `CB` bo'lsa muvaffaqiyat |
| `refunds.commit` | outbox | `money` | `refund.status` `approved`/`processing` bo'lmasa chiqadi |
| `notify.*` | outbox | `default` | Xabar yuborilmasa buyurtmani yiqitmaydi |
...
**Navbat ajratish.** Bugun bitta default navbat bor (`celery_app.py`).
`money` navbati qo'shiladi va ticketing/refund/outbox shunga yo'naltiriladi:
katalog yangilash yoki valyuta kursi kabi statik ish pul yo'lini och
qoldirmasligi kerak.

`task_acks_late=True` va `task_reject_on_worker_lost=True` allaqachon
sozlangan (`celery_app.py:49-51`) — aynan shu yo'l uchun.

> **Beat qoidasi:** beat yozuvi u nomlagan task bilan **bitta commitda**
> qo'shiladi, aks holda beat ishga tushishda yiqiladi (`celery_app.py:57-65`).
...
3. `reprice` (O6): yangi jami > to'langan + tolerantlik bo'lsa → `T11`
   (`price_changed`);
4. `ticketing` chaqiriladi (`payment_method: "deposit"`), javob snapshot'ga;
5. har bir yo'lovchida chipta raqami bo'lsa → `T10`; qisman bo'lsa →
   `needs_attention`;
6. xato bo'lsa `classify()` → `T9` (backoff) yoki `T11`.

### `orders.reconcile_orphans` — "noma'lum" bronlarni topish

`created` da qolgan buyurtma uchun GTS'ga `/v1/orders/list/` so'rovi
yuboriladi: `booking_date_from/to` (buyurtma yaratilgan vaqt ± oyna) +
`passenger` (birinchi yo'lovchi familiyasi). **GTS `offer_id` bo'yicha
filtrlashni qo'llab-quvvatlamaydi** — kolleksiyadagi so'rov parametrlari
ro'yxatida u yo'q.

* Aynan bitta nomzod topilsa va uning `offer_id` si mos kelsa — buyurtmaga
  bog'lanadi, `T2`;
* Nomzod yo'q va yosh > 15 daqiqa — `T3` (`failed`);
* Bir nechta nomzod — `T4` (`needs_attention`), nomzodlar `attention_reason`
  va snapshot'da.
...
| **Baza** | `uq_orders_idempotency_key` · `uq_refunds_idempotency_key` · `uq_payment_transactions_provider_ref` · `uq_orders_provider_number_live` | Migratsiyalar |
...
* `outbox.dispatch` `FOR UPDATE SKIP LOCKED` bilan oladi — bir nechta worker
...
| `outbox` kechikishi (eng eski `pending` yoshi) | Dispatcher tirikmi |
...
| `paid` | 15 daqiqa | `WARNING` + alert (outbox tiqilib qolganmi?) |
...
|---|---|---|---|
| **S1** | `ticket_time_limit` jonli GTS'da qaysi formatda? Uch xil kuzatilgan: ISO datetime, `4319`, `288000` | S2 | Adapter uchalasini ham o'qiydi (§3.5), aniqlanmasa DB'dagi zaxira + `WARNING` |
| **S2** | GTS `cancel` javobida `data` kaliti bormi? Kolleksiyadagi namunada **yo'q** → klient `502` beradi (A7) | S9 | Adapter `cancel` uchun `data` siz javobni ham qabul qiladigan yo'l bilan o'qiydi |
| **S3** | GTS deposit balansini ticketingdan **oldin** o'qiy olamizmi (`/v1/contract/provider/balance/check/`)? | S7 | Yo'q deb hisoblaymiz: balans xatosi `retryable` sinfida ushlanadi |
| **S4** | `ticketing` o'zi qayta narxlaydimi, yoki `reprice_confirm` majburiymi? | S7 | `reprice_check` chaqiriladi; `reprice_confirm` faqat narx o'zgargan bo'lsa |
| **S5** | Qisman qaytarish siyosati (`PROJECT.md` §16.3): jarima qanday hisoblanadi, kim tasdiqlaydi | v2 | `partially_refunded` va `refunds.scope` modelda joy egallaydi, yo'l qurilmaydi |
| **S6** | Anonimlashtirishda qaysi maydonlar moliyaviy hujjat sifatida **saqlanishi shart**? | S11 | Chipta raqami, yo'lovchi turi, summalar qoladi; ism/hujjat/kontakt tozalanadi |
| **S7** | PCI SAQ D majburiyati kimning zimmasida (`PROJECT.md` §16.7) | — | Tijoriy savol, texnik qaror o'zgarmaydi |
| **S8** | Bir buyurtmada bir nechta to'lov kerak bo'ladimi (bo'lib to'lash, qo'shimcha xizmatlar)? | — | v1: bitta (O12). Model buni keyin bo'shatishga to'sqinlik qilmaydi |
...
qo'yilgan har bir qator §3.10 dagi ochiq savol bilan bog'langan** — ular
...
| `cancel` **javobi** (`{status, code, order}` — `data` siz) | `EASY_GATEWAY` → `content/Cancel` | ⚠ **S2** — jonli javob boshqacha bo'lishi mumkin |
...
| `ticket_time_limit` **formati** | Uch xil: ISO (kolleksiya) · `4319` (`drct-error1.json`) · `288000` (`API.md` §20) | ⚠ **S1** — ziddiyatli |
...
| Deposit balansi (`/v1/contract/provider/balance/check/`) | `EASY_GATEWAY` → `agreements/Provider` | ⚠ **S3** — B2C credential'i bilan ochiqmi, noma'lum |
