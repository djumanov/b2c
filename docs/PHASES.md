...
| 4 | `ProductAdapter` porti + `flight` adapteri — **✅ 2026-08-12 (search+offers)** | Port loyihalandi (flow metodlariga `GtsClient` per-call uzatiladi), `flight` adapteri passthrough sifatida qurildi ([ARCHITECTURE.md](ARCHITECTURE.md) §13.8). `upsell` va `verify` ham passthrough bo'lib qo'shildi (2026-08-13); `book` saga bilan keladi |
