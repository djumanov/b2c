...
| 9 | **`booking` sagasi** | Transactional outbox + Celery; eng yuqori xavfli modul. `POST /public/{product}/booking/` **allaqachon ishlaydi** (4/5-bo'lak, sof passthrough) — bu bo'lak uni almashtirmaydi, ustiga buyurtma qatori, `payment_id` va holat mashinasini qo'yadi. Shu yerda `409 offer_expired` xaritasi va `cancel` ning egalik tekshiruvi ham yopiladi (STATUS.md §8.14) |
